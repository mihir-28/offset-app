    ...
    const unsubscribeCycles = onSnapshot(
      ...
      async (snap) => {
        ...
        const decryptedCycles = await Promise.all(
          snap.docs.map((docSnap) => decryptStatementCycleDoc(docSnap.data(), docSnap.id))
        );
        decryptedCycles.forEach((cycle) => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col flex-1 items-center justify-center min-h-[50vh] text-zinc-400">
        ...
        <p className="text-xs tracking-widest text-zinc-400 font-sans uppercase animate-pulse">Loading ledger...</p>
      </div>
    );
  }
