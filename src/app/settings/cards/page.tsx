...
import { archiveCard } from "../../../lib/db-helpers";
import { Button } from "../../../components/ui/button";
import { Input } from "../../../components/ui/input";

export default function CardsSettingsPage() {
  const { cards, activeCard, selectCard, refreshCards, createAndSelectCard } = useAuth();
  const [name, setName] = useState("");
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const addCard = async (event: FormEvent) => { event.preventDefault(); if (!name.trim()) return; setSaving(true); setError(""); try { await createAndSelectCard(name); setName(""); } catch (error) { console.error("Card creation failed:", error); setError((error as { message?: string }).message || "Could not add card. Check your connection and try again."); } finally { setSaving(false); } };
